...
public class MqttController {
    ...
    public synchronized void publishAction(String plugName, String action) throws MqttException {
        String topic = topicPrefix + "/action/" + plugName + "/" + action;
        try {
            client.publish(topic, new MqttMessage());
        } catch (Exception e) {
            logger.error("MqttCtl {}: {} fail to publish", clientId, topicPrefix);
        }
    }
    ...
    synchronized protected void handleUpdate(String topic, MqttMessage msg) {
        ...
        logger.info("name update: {} {} {}", nameUpdate[0], nameUpdate[1], nameUpdate[2]);
        switch (nameUpdate[2]) {
            case "state":
                states.put(nameUpdate[1], new String(msg.getPayload()));

                break;
            case "power":
                powers.put(nameUpdate[1], new String(msg.getPayload()));
                break;
            default:
                return;
        }
